...
Type TMatrix
	...
	Method GetMatrixScale:TVector3()
	
		Local s:TVector3=TVector3.Create()
		s.x[0] = TVector3.Magnitude(grid[(4*0)+0], grid[(4*0)+1], grid[(4*0)+2])
		s.y[0] = TVector3.Magnitude(grid[(4*1)+0], grid[(4*1)+1], grid[(4*1)+2])
		s.z[0] = TVector3.Magnitude(grid[(4*2)+0], grid[(4*2)+1], grid[(4*2)+2])
		Return s
